...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output_pre_proj = attn_output.reshape(bsz, tgt_len, embed_dim)

        attn_output = self.out_proj(attn_output_pre_proj)

        return attn_output, attn_weights_reshaped, qkvs, attn_output_pre_proj
...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, attn_weights, qkvs, _ = self.self_attn(
            hidden_states,
...
class CLIPVisionModelWithProjection(CLIPPreTrainedModel):
    ...
    def get_reuse_map(self):
        reuse_maps = []
        for layer in self.vision_model.encoder.layers:
            last_reuse_map = layer.reuse_module.last_reuse_map.squeeze(0)
            reuse_maps.append(last_reuse_map)
        return torch.stack(reuse_maps)