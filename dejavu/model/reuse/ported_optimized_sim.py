    ...
    def stage_states(
        ...
        for i in range(4):
            ...
            compute_states = torch.gather(
                query_states.reshape(B*(N-1), dim),
                dim=0,
                index=compute_idx
            ) # N'',dim
            compute_states_norm = torch.gather(
                query_states_norm.reshape(B*(N-1), dim),
                dim=0,
                index=compute_idx
            )
            ...
            list_gather_idxs[i,:,1:] = gather_idx
    ...
    def forward(
        ...
        **kwargs, # kwargs["cache"] as caches. h,q,k,v,ref_norm 
        ...
        if not self.is_first_layer:
            ...
            list_gather_idxs, list_hidden_states = list_hidden_states
        else:
            # XXX: or change list_hidden_states to list of tensors
        ...
        if self.is_last_layer:
            ...
            list_hidden_states_norm = list_hidden_states / (list_hidden_states.norm(dim=-1, keepdim=True) + eps)
            ...
            reuse_threshold = self.reuse_thresholds.unsqueeze(0).unsqueeze(0).expand(4, B, N-1)
        ...
        if not self.is_last_layer:
            ...
            list_hidden_states = (list_gather_idxs, list_hidden_states)
