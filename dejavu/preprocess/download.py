...
from ..utils.preprocess import get_available_datasets, get_available_splits, filter_existing_files
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--delete-undecodable",
        action='store_true',
        help='Deletes undecodable videos, need to be used with care'
    )
    args = parser.parse_args()

    if args.DATASET == 'how2qa':
        youtube_ids, video_paths, _ = how2qa.get_youtube_ids_and_paths(split_or_path=args.SPLIT, fps=None)
        start_ends = None
    else:
        raise NotImplementedError(f"Dataset {args.DATASET} is not supported")

    if args.delete_undecodable:
        delete_undecodable_files(Path(PREDEFINED_PATHS[args.DATASET]['video']))
        exit()
    ...
    if len(youtube_ids) > 0:
        download_youtube_videos(youtube_ids, video_paths, start_ends, dry_run=args.dry_run)